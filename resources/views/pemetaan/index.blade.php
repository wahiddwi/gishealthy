...
@foreach ($laykes as $data)
  // menampilkan informasi di map
    var info = `<table><thead><tr><th colspan="2" class="text-center">{{$data->nama_rumahsakit}}</th></tr></thead><tbody><tr><td>Alamat</td><td>: {{$data->alamat}}</td></tr><tr><td>No. Telpon</td><td>: {{$data->no_telpon}}</td></tr><tr></td></tr><tr><td>Jumlah Kamar Tersedia</td><td>: {{App\Kamar::where('id_rumahsakit',$data->id)->where('status',false)->count()}} Kamar</td></tr></td></tr><tr><td colspan="2" class="text-center"><a href="{{route('pemetaan.laykesDetail', $data->id)}}" class="btn btn-sm btn-default">Detail</a></td></tr></tbody></table>`;
    // // menampilkan marker
